...
```bash
npx skills add Andrew-liu/ai-signal -s ai-radar -g
```

Then ask your agent: `What happened in AI today?`

**Live site** → [andrew-liu.github.io/ai-signal](https://andrew-liu.github.io/ai-signal/)
...
AI Signal is an auto-updating 24h radar for AI updates. It does more than fetch AI news. It judges source quality first, merges the same event into a story timeline, has three personas score and review the picks, then uses Scout picks, AI labels, source health, and AI ratio to help you decide:
...
![AI Signal v0.9 timeline screenshot](assets/screenshots/radar-v09-timeline.png)
...
AI Signal was never just about fetching information.
...
AI Signal borrows from modern newsroom workflows. Dumping thousands of items into a page is not useful, so the project turns news handling into a stable pipeline: fetch, deduplicate, filter, review, enrich with status, and generate a static site.
...
1. **Fork** [Andrew-liu/ai-signal](https://github.com/Andrew-liu/ai-signal).
2. **Enable Actions**: enable the workflow on the Actions tab. `update-news.yml` runs at minute 17 each hour and publishes only after tests, data-contract, sensitive-data, and quality gates pass.
3. **(Optional) Add `DEEPSEEK_API_KEY`**: this enables persona reviews, title enhancement, review lines, and better Chinese translations. Without it the pipeline falls back to rules and original titles. The default model is `deepseek-v4-flash`; override it with `DEEPSEEK_MODEL`.
4. **Enable GitHub Pages**: Settings → Pages → select **GitHub Actions** as the source. Only the allowlisted `dist/` artifact is deployed.
5. **Change one line in the skill**: point `BASE_URL` in `skills/radar/SKILL.md` to `https://<your-username>.github.io/ai-signal/data`.

Put custom subscriptions in `feeds/follow.opml`. Vercel also runs the quality gate and publishes only `dist/`. The `?data=` override accepts only HTTPS data from the current site, `andrew-liu.github.io`, and GitHub Raw. Local development uses the same-origin `data/` directory.
...
```bash
git clone https://github.com/Andrew-liu/ai-signal.git
cd ai-signal
python3 -m venv .venv
...
- Runs at minute 17 of every hour by default: `17 * * * *`
...
The "updated at" time in the top-right corner of the live page comes from `generated_at` in `data/latest-24h.json`. If the page is stuck on an old time, first check whether the latest `Update AI Signal Snapshot` run in GitHub Actions succeeded, whether it hit fetch errors, and whether Pages deploys from the branch containing the latest `data/` commit.
...
See [Releases](https://github.com/Andrew-liu/ai-signal/releases) for the full history.
