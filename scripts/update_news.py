...
def filter_waytoagi_payload(payload: dict[str, Any], blocklist: set[str]) -> dict[str, Any]:
    """Apply approved removal requests to every WaytoAGI item before publication."""
    filtered: list[dict[str, Any]] = []
    updates = payload.get("updates_7d")
    if not isinstance(updates, list):
        updates = payload.get("updates_today") if isinstance(payload.get("updates_today"), list) else []
    for update in updates:
        if not isinstance(update, dict):
            continue
        title = str(update.get("title") or "").strip()
        url = normalize_url(str(update.get("url") or payload.get("root_url") or WAYTOAGI_DEFAULT).strip())
        update_date = str(update.get("date") or "").strip()
        source = f"社区更新 · {update_date}" if update_date else "社区更新"
        item_id = make_item_id("waytoagi", source, title, url)
        if is_content_blocked(item_id, url, source, blocklist):
            continue
        if is_content_blocked(item_id, url, "WaytoAGI", blocklist):
            continue
        filtered.append(update)

    latest_date = max((str(item.get("date") or "") for item in filtered), default="") or None
    updates_today = [item for item in filtered if item.get("date") == latest_date] if latest_date else []
    result = dict(payload)
    result.update(
        {
            "latest_date": latest_date,
...
SECRET_LIKE_RE = re.compile(
    ...
    r"\bsk-(?!hynix\b)[A-Za-z0-9_-]{12,}\b|"
    r"(?:api[_-]?key|access[_-]?token|auth[_-]?token|secret|token|xsec_token)=([^\s&]{6,}))",
    re.I,
)
...
SENSITIVE_QUERY_KEYS = SENSITIVE_PUBLIC_KEYS | {"key", "sig", "signature"}
...
SENSITIVE_PUBLIC_KEY_NAMES = {normalize_sensitive_name(key) for key in SENSITIVE_PUBLIC_KEYS}
...
def sanitize_public_url(value: str) -> str:
    """Strip credentials, fragments, and secret-like query parameters from public URLs."""
    try:
...
def sanitize_public_value(value: Any, field_name: str = "") -> Any:
    normalized_field = normalize_sensitive_name(field_name)
    if normalized_field in SENSITIVE_PUBLIC_KEY_NAMES:
        return None
    if isinstance(value, str):
        redacted = redact_public_text(value)
        if is_http_url(redacted):
            return sanitize_public_url(redacted)
        if normalized_field in {"summary", "preview", "description"} and len(redacted) > 360:
            return redacted[:359].rstrip() + "…"
        return redacted
    if isinstance(value, list):
        return [sanitize_public_value(item) for item in value]
    if isinstance(value, dict):
        return {key: sanitize_public_value(val, str(key)) for key, val in value.items()}
    return value
...
def load_translation_glossary(
    ...
    try:
        ...
        try:
            glossary_text = raw_bytes.decode("utf-8")
        except UnicodeDecodeError:
            glossary_text = raw_bytes.decode("gb18030")
...
def load_content_blocklist(path: str | Path) -> set[str]:
    """Load exact item IDs, normalized URLs, or source labels to suppress."""
    blocklist_path = Path(path)
    if not blocklist_path.is_absolute():
        blocklist_path = Path(__file__).resolve().parent.parent / blocklist_path
    try:
        return {
            line.strip()
...
def is_content_blocked(item_id: str, url: str, source: str, blocklist: set[str]) -> bool:
    return bool(
        item_id in blocklist
        or normalize_url(url) in blocklist
        or str(source or "").strip() in blocklist
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--content-blocklist",
        default="content-blocklist.txt",
        help="Exact item IDs, normalized URLs, or source labels that must never be published",
    )
    args = parser.parse_args()

    now = utc_now()
    content_blocklist = load_content_blocklist(args.content_blocklist)
    ...
    try:
        ...
        waytoagi_payload = filter_waytoagi_payload(waytoagi_payload, content_blocklist)
    ...
    for raw in raw_items:
        ...
        if is_content_blocked(item_id, url, raw.source, content_blocklist):
            archive.pop(item_id, None)
            continue
    ...
    if email_digest_payload is not None:
        atomic_write_json(email_digest_path, sanitize_public_payload(email_digest_payload), indent=2)
    atomic_write_json(waytoagi_path, sanitize_public_payload(waytoagi_payload), indent=2)
    atomic_write_json(title_cache_path, sanitize_public_payload(title_cache), indent=2)
