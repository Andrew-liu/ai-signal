...
// 数据同源开关仅接受当前站点、项目 GitHub Pages、GitHub Raw 和本机开发地址。
...
function buildEventSourceRow(source) {
  ...
  titleLink.href = safeExternalUrl(source.url);
...
function renderSourceHealthSummaryNode(status, errorMessage = "") {
  ...
  segments.push(`失败 ${fmtNumber(failed)}`);
  detail.textContent = segments.join(" · ");
  if (failed > 0) detail.classList.add("source-health-fail-bad");
  node.append(title, detail);
  return node;
}
