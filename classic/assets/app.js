...
function safeDataBaseUrl(value) {
  ...
  try {
    ...
    if (!TRUSTED_DATA_HOSTS.has(url.hostname)) return "";
    url.username = "";
    url.password = "";
    url.search = "";
    url.hash = "";
...
// Keep both UI surfaces on the same validated snapshot.
function resolveDataBaseUrl() {
  ...
  if (fromQuery) {
    const normalized = safeDataBaseUrl(fromQuery);
    try {
      if (normalized) localStorage.setItem("dataBaseUrl", normalized);
      else localStorage.removeItem("dataBaseUrl");
    } catch {}
    return normalized;
  }
  try {
    const normalized = safeDataBaseUrl(localStorage.getItem("dataBaseUrl") || "");
    if (!normalized) localStorage.removeItem("dataBaseUrl");
    return normalized;
  } catch {
    return "";
  }
...
function buildBoleLead(row) {
  ...
  lead.href = safeExternalUrl(item.url);
  lead.target = "_blank";
  ...
  [item.site_name || "来源", item.source || "未分区"].forEach((value) => {
...
function buildBoleTimelineRow(row, rank) {
  ...
  rankMeta.textContent = `#${rank}`;
  const sourceMeta = document.createElement("span");
  sourceMeta.textContent = item.site_name || "来源";
  const scoreMeta = document.createElement("strong");
  scoreMeta.textContent = `${score}分`;
  meta.append(rankMeta, sourceMeta, scoreMeta);
...
function buildStoryCard(story, rank) {
  ...
  link.href = safeExternalUrl(primary.url || story.primary_url || story.url);
  link.target = "_blank";
  link.rel = "noopener noreferrer";
...
function renderItemNode(item, context = {}) {
  ...
  titleEl.href = safeExternalUrl(item.url);
...
function renderWaytoagi(waytoagi) {
  ...
  rootLink.href = safeExternalUrl(waytoagi.root_url);
  rootLink.target = "_blank";
  rootLink.rel = "noopener noreferrer";
  rootLink.textContent = "主页面";
  const historyLink = document.createElement("a");
  historyLink.href = safeExternalUrl(waytoagi.history_url);
  ...
  updates.forEach((u) => {
    ...
    row.href = safeExternalUrl(u.url);
    row.target = "_blank";
...
function renderSourceHealthSummaryNode(status, errorMessage = "") {
  ...
  if (!status) {
    ...
    title.textContent = errorMessage ? "源状态异常" : "源状态未生成";
    detail.textContent = errorMessage || "等待 source-status.json";
  ...
  title.textContent = `${fmtNumber(okSites)}/${fmtNumber(sites.length)} 源正常`;
  detail.textContent = `今日采集 ${fmtNumber(fetched)} 条 · 失败 ${fmtNumber(failed)}`;
  node.append(title, detail);
  return node;
}
...
function renderSourceStatusTable(status) {
  ...
  ["来源", "AI / 原始", "AI占比", "状态"].forEach((label) => {
    const cell = document.createElement("span");
  ...
  rows.forEach((site) => {
    ...
    const values = [
      site.site_name || site.site_id || "未知来源",
      `${fmtNumber(site.aiCount)} / ${fmtNumber(site.rawCount)}`,
      `${fmtNumber(site.ratio)}%`,
      site.ok ? "正常" : "异常",
    ];
    values.forEach((value, index) => {
      const cell = document.createElement("span");
      cell.textContent = value;
      if (index === 3) cell.className = site.ok ? "ok" : "bad";
...
async function init() {
  ...
  if (newsResult.status === "fulfilled") {
    ...
    error.textContent = String(newsResult.reason?.message || "新闻数据加载失败");
    newsListEl.appendChild(error);
    renderCoverageStrip(newsResult.reason?.message || "新闻数据加载失败");
  }
  ...
  if (waytoagiResult.status === "fulfilled") {
    ...
    const error = document.createElement("div");
    error.className = "waytoagi-error";
    error.textContent = String(waytoagiResult.reason?.message || "社区数据加载失败");
    waytoagiListEl.appendChild(error);
  }
