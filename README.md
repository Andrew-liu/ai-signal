...
装完对 Agent 说一句：`今天 AI 圈有什么？`

**在线站** → [andrew-liu.github.io/ai-signal](https://andrew-liu.github.io/ai-signal/)

[English](README.en.md) · [雷达 Skill](skills/radar/README.md) · [伯乐 Skill](skills/ai-news-radar/README.md) · [信息源策略](docs/SOURCE_COVERAGE.md)
...
AI Signal是一个自动更新的24小时AI更新雷达。它不只是把AI新闻抓回来，会先判断信息源质量，把同一个事件合并成故事线，再用三种口味的 persona 替你打分点评，最后用伯乐精选、AI标签、源健康和AI占比帮你判断：
...
![AI Signal v0.9 时间轴截图](assets/screenshots/radar-v09-timeline.png)
...
AI Signal学习了现代新闻学的技术，不是简单堆信息源，一次性放几万条信息出来等于没用，所以我选择把新闻处理拆成稳定pipeline，抓取，去重，过滤，锐评，补充状态，生成静态站点。
...
2. **开 Actions**：在 Actions 页启用 workflow。`update-news.yml` 每小时第 17 分钟运行，候选快照通过测试、数据契约、敏感信息和质量门禁后才会发布。
3. **（可选）配 `DEEPSEEK_API_KEY`**：Settings → Secrets and variables → Actions 加一个 secret，就能获得 persona 锐评、标题增强、精选条目的真实推荐理由，以及更可靠的中文标题翻译。不配也能运行，自动降级成规则分和原始标题。默认模型是 `deepseek-v4-flash`，可用 Variable `DEEPSEEK_MODEL` 覆盖。
4. **开 GitHub Pages**：Settings → Pages → Source 选择 **GitHub Actions**。工作流只部署 `dist/` 白名单产物。
5. **改 Skill 一行**：把 `skills/radar/SKILL.md` 顶部的 `BASE_URL` 换成 `https://<你的用户名>.github.io/ai-signal/data`。

想换信源：把订阅写进 `feeds/follow.opml`，参考 `feeds/follow.example.opml`。想换口味：改 `personas/` 下的 Markdown。Vercel 也会先执行质量门禁，再只发布 `dist/`。`?data=` 仅接受当前站点、`andrew-liu.github.io` 和 GitHub Raw 的 HTTPS 地址，不能加载任意第三方数据源；本地开发直接使用同源 `data/`。
...
- 默认每小时第 17 分钟运行一次：`17 * * * *`
- 先在临时候选目录生成数据，执行 pytest、结构校验、敏感信息扫描、来源成功率、数据新鲜度和数量突降门禁；失败时保留上一版
- 门禁通过后原子提升到 `data/`，并只把 `dist/` 白名单制品部署到 GitHub Pages
...
- SocialData/TikHub 的拉取间隔会记录在 `data/paid-source-state.json`，只保存上次运行时间、结果数和错误名，不保存 API Key；小时工作流跳过付费源时，旧条目仍保留在 `data/archive.json`，不会因为本轮未拉取就被清空

默认情况下，本项目不需要任何API Key就能跑核心流程。

线上页面右上角显示的"更新时间"来自 `data/latest-24h.json` 的 `generated_at`。如果页面长时间停在旧时间，优先检查 GitHub Actions 最近一次 `Update AI Signal Snapshot` 是否运行、是否有抓取错误、以及仓库 Pages 是否部署到包含最新 `data/` 提交的分支。
...
历次改动见 [Releases](https://github.com/Andrew-liu/ai-signal/releases)。
